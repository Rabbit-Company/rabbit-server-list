export class MinecraftVoteDO{
	...
	async fetch(request) {
		...
		if(currentAlarm == null){
			let nextMonth = new Date(new Date().getFullYear(), new Date().getMonth() + 1, 1).getTime() + 300_000;
      this.state.storage.setAlarm(nextMonth);
		...
		if(url.pathname === '/vote'){
			...
			if(!data['username']) return MinecraftVoteDO.jsonResponse({ 'error': 1000, 'info': 'Not all required data provided in json format.' });

			let ipVoteDate = await this.state.storage.get('votedate-ip-' + ip);
			if(ipVoteDate !== null){
			...
			await this.state.storage.put('votedate-ip-' + ip, Date.now());
